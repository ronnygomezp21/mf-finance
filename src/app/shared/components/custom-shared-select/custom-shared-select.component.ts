...
import { ModeSelectEnum, PaginationEnum } from '../../enum';
import { IPagination } from '../../interfaces/catalog.interface';

@Component({
  standalone: false,
  selector: 'mf-core-shared-custom-select',
  templateUrl: './custom-shared-select.component.html',
})
export class CustomSharedSelectComponent
  implements OnInit, OnChanges, AfterViewInit, OnDestroy
{
  ...
  customRequiredLabel = input<string>();
  ...
  ngDoCheck(): void {
    ...
    if (
      ...
      if (controller) {
        this.copyValidators(this.control(), controller);
      }
    }
  ...
  private _onSearchedDebounced(event: { term: string; items: any[] }) {
    ...
    const tooLittleItems = event.items.length < 4;
    if (!tooLittleItems) {
      return;
    }
  ...
  pushChips(value: any) {
    if (value) {
      switch (this.isMode) {
        case ModeSelectEnum.COMBO:
          const option = value[this.bindValue];
